...
for package in "${dependencies[@]}"; do
    ...
    if [[ "${currentPackage[0]}" == "@@" ]]; then
        packageType="${currentPackage[1]}"
        packageName="${currentPackage[2]}"
        # Supported types: 'arch', 'aur', 'makepkg', 'pip'
        if [[ "$type" == "pip" ]]; then
            if [[ ! $(pacman -Q "python" &>/dev/null) ]]; then
                missingPackages+="$currentPackage" # python not installed
            else
                python -c "import proton-keyring-linux" && echo $? > result
                [ ! $result ] && missingPackages+="$currentPackage" # python package not found
            fi
        elif [[ ! $(pacman -Q "$packageName" &>/dev/null) ]]; then
            missingPackages+="$currentPackage" # package not found with pacman
        fi
    fi
done
